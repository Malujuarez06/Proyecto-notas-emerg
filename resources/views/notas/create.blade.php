...
@section('content')
    <h1 class="text-2xl font-semibold mb-4">Crear Nueva Nota</h1>
    ...
    <form action="{{ route('notas.store') }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="titulo" id="titulo" class="input input-bordered w-full" value="{{ old('titulo') }}" required>
            @error('titulo')
        ...
        <div class="mb-4">
            ...
            <textarea name="contenido" id="contenido" class="textarea textarea-bordered w-full" rows="5" required>{{ old('contenido') }}</textarea>
            @error('contenido')
                <div class="text-red-500 text-sm mt-2">{{ $message }}</div>
            @enderror
        </div>
